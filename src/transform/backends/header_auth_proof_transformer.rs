...
use crate::{authentication::AuthProof, transform::AuthProofTransformer};


/// A stateless [crate::transform::AuthProofTransformer] implementation that extracts authentication proof from a http header
#[derive(Debug, Clone)]
...
impl HeaderAuthProofTransformer {
    /// Creates a new instance of HeaderAuthProofTransformer with the provided header used for authentication proof
    /// extraction
...
#[async_trait]
impl<AuthnProof: AuthProof + 'static + TryFrom<Vec<u8>>> AuthProofTransformer<AuthnProof>
    for HeaderAuthProofTransformer
where
    AuthnProof::Error: std::error::Error,
{
    ...
    async fn insert_auth_proof(&mut self, mut request: Request) -> Result<Request, Self::Error> {
        if let Some(header) = request.headers().get(&self.header) {
            let auth_proof: Result<AuthnProof, <AuthnProof as TryFrom<Vec<u8>>>::Error> =
    ...
    async fn process_auth_state_change(
        ...
        // We do not process any auth state changes as this AuthProofTransformer is
        Ok(response)
    }
}